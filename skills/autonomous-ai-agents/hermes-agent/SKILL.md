...
4. **404 Errors (Google Gemini)**:
    *   If you get a 404 with the `google` provider, first ensure your `config.yaml` doesn't have a conflicting `model.base_url` or `model.api_mode` that might be overriding the Google endpoint.
        *   Example fix: Ensure `model.default` specifies only the model, and separate provider-specific configs (`providers.google`, `providers.custom`) define `base_url` and `api_mode` appropriately.
    *   Next, verify the model name itself. The error "models/<name> is not found for API version v1beta" indicates the model name used might be incorrect, deprecated, or not supported for `generateContent` in the API version Hermes uses. Consult Google's official API documentation for current model names and their compatibility with `v1beta`.
    *   If the model name is correct, the 404 could be due to temporary API unavailability or throttling (check for `RESOURCE_EXHAUSTED` errors).

5. **403 PERMISSION_DENIED (Google Gemini)**:
    *   If you receive a `403 PERMISSION_DENIED` error when trying to use Google Gemini, this means your `GOOGLE_API_KEY` (or `GEMINI_API_KEY`) is likely invalid, revoked, or lacks the necessary permissions to access the Gemini API.
    *   **Action**: Generate a new API key from Google AI Studio (https://aistudio.google.com/app/apikey) and ensure it has full access to the Gemini API. Replace the old key in your `~/.hermes/.env` file using `hermes config set --env GOOGLE_API_KEY`.
6. **Provider Mismatch or `config.yaml` Conflicts (Multiple LLMs)**:
    *   When using multiple LLM providers (e.g., local Ollama and Google Gemini), ensure your `config.yaml` is set up to handle each provider's `base_url` and `api_mode` correctly.
        *   Example correct `config.yaml` structure:
        ```yaml
        model:
          default: ollama/gemma3:4b # Default model, could be Ollama or another
          provider: custom # Refers to a provider defined below

        providers:
          custom: # Configuration for Ollama
            provider: custom
            base_url: http://localhost:11434/v1
            api_mode: chat_completions
          google: # Configuration for Google Gemini
            provider: gemini
            api_key: ${GOOGLE_API_KEY} # Uses the API key from .env
        ```
    *   Avoid setting `base_url` or `api_mode` directly under the top-level `model:` section if you intend to use `hermes chat --provider <name>`. These should be defined under their specific provider entry in the `providers:` section.
...
### Agent Self-Awareness & Memory

**"MEMORY (your personal notes) [Last updated: YYYY-MM-DD]" Timestamp:**
- This timestamp indicates the *last time an explicit write/modification operation was performed on the persistent memory* using the `memory` tool.
- It does *not* reflect the agent's general awareness of the current date or its ability to recall facts from the current session or session history.
- The agent has access to real-time date/time information from the system and retains all session context.
```
