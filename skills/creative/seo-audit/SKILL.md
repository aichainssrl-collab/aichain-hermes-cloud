...
This skill may use external tools that consume a significant number of API calls or web requests. To avoid exceeding API limits (e.g., Gemini 429 errors), consider the following:

*   Limit the use of external tools where possible.
*   Use internal tools or APIs that are more efficient.
*   Focus on the most important aspects of SEO to avoid unnecessary analysis.
...
- For analytical and nuanced tasks, especially those requiring strategic insights (like SEO audits), ensure the selected model provides sufficient quality.
- Lite models (e.g., `gemini-2.5-flash-lite`) might result in a "calo di prestazioni" (drop in performance/quality) if the task demands deeper reasoning or more comprehensive output.
- *User Preference:* The Boss prefers powerful models (e.g., full Gemini) for complex analytical and programming tasks to ensure maximum potency and quality.
- Consider switching to a more capable model if the output quality for a given task is deemed insufficient.
...
- **messaging-troubleshooting**: For troubleshooting issues related to sending messages via different platforms (e.g., Telegram).
