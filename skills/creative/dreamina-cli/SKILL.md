...
Install from the official ByteDance endpoint:

```bash
curl -s https://jimeng.jianying.com/cli | bash
```

Or manually download the binary and SKILL.md from the `dreamina_cli_beta` path on `lf3-static.bytednsdoc.com`. The CLI is available for `linux_arm64`, `linux_amd64`, and `windows_amd64`.

## Pitfalls & Known Issues

- **Authentication is China-centric:** `dreamina login --headless` uses OAuth Device Flow that redirects to `jimeng.jianying.com/ai-tool/cli-auth`. This typically requires a Douyin (抖音) or ByteDance account with Chinese phone verification. It **does not work** with standard Google/Apple social login from outside China.
- **Slow downloads from Italy:** The binary is hosted on a Chinese CDN (`lf3-static.bytednsdoc.com`). Direct `curl | bash` may timeout; download the binary and SKILL.md separately with generous timeouts.
- **Credits are consumed per generation:** Always check `dreamina user_credit` before submitting tasks. The service is not free beyond initial trial credits.
- **Better alternative for users outside China:** [Replicate](https://replicate.com) (`pip install replicate`) provides access to the same tier of models (CogVideoX, Luma, Kling, Mochi, Stable Video Diffusion) with a simple API key that works globally. No QR login, no Chinese phone number required.
