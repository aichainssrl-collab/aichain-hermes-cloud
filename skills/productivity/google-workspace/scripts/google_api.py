...
def drive_create_folder(args):
    ...
    if _gws_binary():
        ...
        print(json.dumps({"id": result.get("id", ""), "name": result.get("name", ""), "webViewLink": result.get("webViewLink", "")}, indent=2, ensure_ascii=False))
        return

    service = build_service("drive", "v3")
    
    # Check for existing folder with same name in parent
    if args.parent:
        existing = service.files().list(
            q=f"name='{args.name}' and mimeType='application/vnd.google-apps.folder' and '{args.parent}' in parents and trashed=false",
            fields="files(id, name)",
        ).execute()
        folders = existing.get("files", [])
        if folders:
            folder = folders[0]
            print(json.dumps({"id": folder["id"], "name": folder["name"], "alreadyExisted": True, "webViewLink": f"https://drive.google.com/drive/folders/{folder['id']}"}, indent=2, ensure_ascii=False))
            return
    ...
    if args.parent:
        file_metadata["parents"] = [args.parent]

    folder = service.files().create(body=file_metadata, fields="id, name, webViewLink").execute()
    print(json.dumps({"id": folder.get("id"), "name": folder.get("name"), "webViewLink": folder.get("webViewLink")}, indent=2, ensure_ascii=False))


def _drive_create_file(args, mime, extension):
    """Create a Google file (Doc or Sheet) with content, saved to a specific folder."""
    service = build_service("drive", "v3")
    
    file_metadata = {
        "name": args.name if hasattr(args, 'name') and args.name else f"file{extension}",
        "parents": [args.parent] if hasattr(args, 'parent') and args.parent else [],
    }
    ...
    if hasattr(args, 'content') and args.content:
        ...
        if hasattr(args, 'base64') and args.base64:
            import base64
        ...
        media = MediaFileUpload(tmp.name, mimetype='application/octet-stream', resumable=True)
    ...
    # If it's a Google Doc, import the content
    if mime == "application/vnd.google-apps.document":
        # Delete the raw file and create a proper Google Doc
        doc_metadata = {
            "name": file_metadata["name"],
            "mimeType": "application/vnd.google-apps.document",
            "parents": file_metadata.get("parents", []),
        }
        # Import markdown as a Google Doc via Drive API
        if hasattr(args, 'content') and args.content:
            tmp = tempfile.NamedTemporaryFile(mode='w', suffix='.md', delete=False, encoding='utf-8')
            tmp.write(args.content)
            tmp.close()
            
            import_mime = "text/markdown"
            media = MediaFileUpload(tmp.name, mimetype=import_mime, resumable=True)
            doc_file = service.files().create(body=doc_metadata, media_body=media, fields="id, name, webViewLink, mimeType").execute()
            os.unlink(tmp.name)
            
            # Also delete the raw file we created first
            service.files().delete(fileId=file.get("id")).execute()
            
            print(json.dumps({"id": doc_file.get("id"), "name": doc_file.get("name"), "webViewLink": doc_file.get("webViewLink"), "mimeType": doc_file.get("mimeType")}, indent=2, ensure_ascii=False))
            return

    print(json.dumps({"id": file.get("id"), "name": file.get("name"), "webViewLink": file.get("webViewLink"), "mimeType": file.get("mimeType")}, indent=2, ensure_ascii=False))


def drive_create_doc(args):
    """Create a Google Doc with the given content in a folder."""
    _drive_create_file(args, "application/vnd.google-apps.document", ".md")


def drive_upload_file(args):
    ...
    file_name = args.name if hasattr(args, 'name') and args.name else os.path.basename(file_path)
    ...
    mime_type, _ = _mime.guess_type(file_path)
...
def main():
    ...
    p = drv_sub.add_parser("create-doc")
    p.add_argument("--name", default="document.md", help="File name")
    p.add_argument("--parent", default="", help="Parent folder ID")
    p.add_argument("--content", default="", help="File content (markdown)")
    p.set_defaults(func=drive_create_doc)

    p = drv_sub.add_parser("upload")
    p.add_argument("file_path", help="Local file path to upload")
    p.add_argument("--name", default="", help="Override file name on Drive")
    p.add_argument("--parent", default="", help="Parent folder ID")
    p.set_defaults(func=drive_upload_file)
